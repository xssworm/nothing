<?php
    ...
    function initBuildTemplateInstance($templatePath)
    {
		...
		if (!$GLOBALS['Debug']) {
			$templateOptions['cache'] = getAdminTwigTemplateCacheDirectory();
		}
		
        $template = Interspire_Template::getInstance('admin', $templatePath, $templateOptions);
    ...
    if (GetConfig('isSetup')) {
        ...
        $LicenseType = intval(GetConfig('LicenseType'));
        if (in_array($LicenseType, array(1, 2, 4, 8))) {
            $GLOBALS['ProductEdition'] = GetLang('Edition'.$LicenseType);
        } else {
            $GLOBALS['ProductEdition'] = GetLang('Edition1');
        }
        unset($LicenseType);
    ...
    } elseif (!defined('NO_UPGRADE_CHECK')) {
